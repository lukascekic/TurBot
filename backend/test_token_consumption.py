...
class TokenConsumptionTracker:
    ...
    def __init__(self):
        ...
        # GPT-4o-mini pricing (as of June 2025) - FINAL CORRECT CALCULATION
        self.input_token_cost = 0.00015 / 1000   # $0.00015 per 1K tokens = $0.00000015 per token
        self.output_token_cost = 0.0006 / 1000   # $0.0006 per 1K tokens = $0.0000006 per token
    ...
    def start_tracking(self):
        ...
        print(f"💰 Pricing: ${self.input_token_cost * 1000:.6f}/1K input tokens, ${self.output_token_cost * 1000:.6f}/1K output tokens")
        print("=" * 60)
    ...
    def track_query(self, query_num: int, query: str, response_data: Dict[str, Any], response_time: float):
        ...
        print(f"   Cost: ~${estimated_cost:.6f} (THIS SINGLE QUERY)")
        print(f"   Running Total: ${self.total_cost:.6f} ({query_num} queries so far)")
    ...
    def finish_tracking(self):
        ...
        # Extrapolations based on ACTUAL total cost
        average_cost_per_query = self.total_cost / len(TEST_QUERIES)
        cost_per_1000 = average_cost_per_query * 1000
        queries_for_15_dollars = 15 / average_cost_per_query if self.total_cost > 0 else float('inf')
        
        print(f"📈 EXTRAPOLATIONS (Based on {len(TEST_QUERIES)} actual queries):")
        print(f"   Total cost for {len(TEST_QUERIES)} queries: ${self.total_cost:.6f}")
        print(f"   Average cost per query: ${average_cost_per_query:.6f}")
        ...
        if standard_queries:
            ...
            print(f"   Average Cost: ${avg_standard_cost:.6f} per query")
        ...
        if detailed_queries:
            ...
            print(f"   Average Cost: ${avg_detailed_cost:.6f} per query")
        ...
        return {
            ...
            'cost_breakdown': {
                ...
                'standard_avg_cost': avg_standard_cost if standard_queries else 0,
                'detailed_avg_cost': avg_detailed_cost if detailed_queries else 0
            }
        }
...
async def run_token_consumption_test():
    ...
    print(f"💰 Total cost for all queries: ${summary['total_cost']:.6f}")
    print(f"📊 Average cost per query: ${summary['average_cost_per_query']:.6f}")
    print(f"📈 Extrapolated cost for 1,000 queries: ${summary['cost_per_1000_queries']:.2f}")
    print(f"🎯 Queries possible with $15 budget: {summary['queries_for_15_dollars']:,.0f}")
    print("=" * 60)
